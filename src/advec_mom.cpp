	...
	if (mom_sweep == 1) { // x 1
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class APPEND_LN(advec_mom_x1)>(h, policy, [=](id<2> idx) {
				post_vol[idx[0]][idx[1]] = volume[idx[0]][idx[1]] + vol_flux_y[offset(idx, 0, 1)] - vol_flux_y[idx[0]][idx[1]];
				pre_vol[idx[0]][idx[1]] = post_vol[idx[0]][idx[1]] + vol_flux_x[offset(idx, 1, 0)] - vol_flux_x[idx[0]][idx[1]];
			});
		});
		...
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class APPEND_LN(advec_mom_y1)>(h, policy, [=](id<2> idx) {
				post_vol[idx[0]][idx[1]] = volume[idx[0]][idx[1]] + vol_flux_x[offset(idx, 1, 0)] - vol_flux_x[idx[0]][idx[1]];
				pre_vol[idx[0]][idx[1]] = post_vol[idx[0]][idx[1]] + vol_flux_y[offset(idx, 0, 1)] - vol_flux_y[idx[0]][idx[1]];
			});
		});
	...
	if (direction == 1) {
			...
			clover::execute(q, [&](handler &h) {
				...
				clover::par_ranged<class advec_mom_dir1_vel1_node_flux>(
						h, {x_min - 2 + 1, y_min + 1, x_max + 2 + 2, y_max + 1 + 2}, [=](id<2> idx) {
							...
							node_flux[idx[0]][idx[1]] = 0.25 * (mass_flux_x[offset(idx, 0, -1)] + mass_flux_x[idx[0]][idx[1]]
			...
			clover::execute(q, [&](handler &h) {
				...
				clover::par_ranged<class advec_mom_dir1_vel1_node_mass_pre>(
						h, {x_min - 1 + 1, y_min + 1, x_max + 2 + 2, y_max + 1 + 2}, [=](id<2> idx) {
							...
							node_mass_post[idx[0]][idx[1]] = 0.25 * (density1[offset(idx, 0, -1)] * post_vol[offset(idx, 0, -1)]
							                              + density1[idx[0]][idx[1]] * post_vol[idx[0]][idx[1]]
							                              + density1[offset(idx, -1, -1)] *
							                                post_vol[offset(idx, -1, -1)]
							                              +
							                              density1[offset(idx, -1, 0)] * post_vol[offset(idx, -1, 0)]);
							node_mass_pre[idx[0]][idx[1]] =
									node_mass_post[idx[0]][idx[1]] - node_flux[offset(idx, -1, 0)] + node_flux[idx[0]][idx[1]];
						});
			});
		}
		...
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class advec_mom_dir1_mom_flux>(
					h, {x_min - 1 + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](id<2> idx) {
						...
						sigma = sycl::fabs(node_flux[idx[0]][idx[1]]) / (node_mass_pre[donor][k]);
						width = celldx[j];
						...
						mom_flux[idx[0]][idx[1]] = advec_vel_s * node_flux[idx[0]][idx[1]];
					});
		});
		...
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class advec_mom_dir1_vel1>(
					h, {x_min + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](id<2> idx) {
						vel1[idx[0]][idx[1]] = (vel1[idx[0]][idx[1]] * node_mass_pre[idx[0]][idx[1]] + mom_flux[offset(idx, -1, 0)] -
						             mom_flux[idx[0]][idx[1]]) /
						            node_mass_post[idx[0]][idx[1]];
					});
		});
	} else if (direction == 2) {
		if (which_vel == 1) {
			...
			clover::execute(q, [&](handler &h) {
				...
				clover::par_ranged<class advec_mom_dir2_vel1_node_flux>(
						h, {x_min + 1, y_min - 2 + 1, x_max + 1 + 2, y_max + 2 + 2}, [=](id<2> idx) {
							...
							node_flux[idx[0]][idx[1]] = 0.25 * (mass_flux_y[offset(idx, -1, 0)] + mass_flux_y[idx[0]][idx[1]]
				...
				clover::par_ranged<class advec_mom_dir2_vel1_node_mass_pre>(
						h, {x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 2 + 2}, [=](id<2> idx) {
							node_mass_post[idx[0]][idx[1]] = 0.25 * (density1[offset(idx, 0, -1)] * post_vol[offset(idx, 0, -1)]
							                              ...
							                              density1[offset(idx, -1, 0)] * post_vol[offset(idx, -1, 0)]);
							node_mass_pre[idx[0]][idx[1]] =
									node_mass_post[idx[0]][idx[1]] - node_flux[offset(idx, 0, -1)] + node_flux[idx[0]][idx[1]];
						});
			});
		}
		...
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class advec_mom_dir2_mom_flux>(
					h, {x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](id<2> idx) {
						...
						sigma = sycl::fabs(node_flux[idx[0]][idx[1]]) / (node_mass_pre[j][donor]);
						width = celldy[k];
		...
		clover::execute(q, [&](handler &h) {
			...
			clover::par_ranged<class advec_mom_dir2_vel1>(
					h, {x_min + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2}, [=](id<2> idx) {
						vel1[idx[0]][idx[1]] = (vel1[idx[0]][idx[1]] * node_mass_pre[idx[0]][idx[1]] + mom_flux[offset(idx, 0, -1)] - mom_flux[idx[0]][idx[1]]) /
						            node_mass_post[idx[0]][idx[1]];
					});
		});
	}
