	...
	if (dir == g_xdir) {
		...
		if (sweep_number == 1) {
			...
			const int k) {
				...
				pre_vol(j, k) = volume(j, k) +
				                (vol_flux_x(j + 1, k) - vol_flux_x(j, k) + vol_flux_y(j, k + 1) -
				                 vol_flux_y(j, k));
		...
		// DO k=y_min,y_max
		//   DO j=x_min,x_max+2
		Kokkos::MDRangePolicy <Kokkos::Rank<2>> policy_x2({x_min + 1, y_min + 1},
		                                                  {x_max + 2 + 2, y_max + 2});
		Kokkos::parallel_for("advec_cell xdir ener_flux", policy_x2, KOKKOS_LAMBDA(
		const int j,
		const int k) {
		...
		// DO k=y_min,y_max
		//   DO j=x_min,x_max
		Kokkos::MDRangePolicy <Kokkos::Rank<2>> policy_xy({x_min + 1, y_min + 1},
		                                                  {x_max + 2, y_max + 2});
		Kokkos::parallel_for("advec_cell xdir density1,energy1", policy_xy, KOKKOS_LAMBDA(
		const int j,
		const int k) {
			double pre_mass_s = density1(j, k) * pre_vol(j, k);
			double post_mass_s = pre_mass_s + mass_flux_x(j, k) - mass_flux_x(j + 1, k);
			double post_ener_s =
					(energy1(j, k) * pre_mass_s + ener_flux(j, k) - ener_flux(j + 1, k)) /
					post_mass_s;
		...
		if (sweep_number == 1) {
			Kokkos::parallel_for("advec_cell ydir sweep_number=1", policy, KOKKOS_LAMBDA(
			const int j,
			const int k) {

				pre_vol(j, k) = volume(j, k) +
				                (vol_flux_y(j, k + 1) - vol_flux_y(j, k) + vol_flux_x(j + 1, k) -
				                 vol_flux_x(j, k));
		...
		// DO k=y_min,y_max+2
		//   DO j=x_min,x_max
		Kokkos::MDRangePolicy <Kokkos::Rank<2>> policy_y2({x_min + 1, y_min + 1},
		                                                  {x_max + 2, y_max + 2 + 2});
		Kokkos::parallel_for("advec_cell ydir ener_flux", policy_y2, KOKKOS_LAMBDA(
		const int j,
		const int k) {
		...
		Kokkos::MDRangePolicy <Kokkos::Rank<2>> policy_xy({x_min + 1, y_min + 1},
		                                                  {x_max + 2, y_max + 2});
		Kokkos::parallel_for("advec_cell ydir density1,energy1", policy_xy, KOKKOS_LAMBDA(
		const int j,
		const int k) {
			...
			double post_ener_s =
					(energy1(j, k) * pre_mass_s + ener_flux(j, k) - ener_flux(j, k + 1)) /
					post_mass_s;
