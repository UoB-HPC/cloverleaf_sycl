...
void hydro(global_variables &globals, parallel_ &parallel) {
  ...
  if (DEBUG)
    globals.dump("dumps/dump_" + std::to_string(parallel.task) + "_" + std::to_string(globals.step) + "_0.txt");
  ...
  while (true) {
    ...
    if (DEBUG)
      globals.dump("dumps/dump_" + std::to_string(parallel.task) + "_" + std::to_string(globals.step) +
                   "_1_timestep.txt");

    PdV(globals, true);
    if (DEBUG)
      globals.dump("dumps/dump_" + std::to_string(parallel.task) + "_" + std::to_string(globals.step) + "_2_PdV.txt");
    ...
    PdV(globals, false);
    if (DEBUG)
      globals.dump("dumps/dump_" + std::to_string(parallel.task) + "_" + std::to_string(globals.step) + "_4_PdV.txt");
    ...
    if (globals.time + g_small > globals.config.end_time || globals.step >= globals.config.end_step) {
      ...
      if (globals.profiler_on) {
        ...
        double kernel_total = globals.profiler.timestep + globals.profiler.ideal_gas + globals.profiler.viscosity +
                              globals.profiler.PdV + globals.profiler.revert + globals.profiler.acceleration +
                              globals.profiler.flux + globals.profiler.cell_advection + globals.profiler.mom_advection +
                              globals.profiler.reset + globals.profiler.summary + globals.profiler.visit +
                              globals.profiler.tile_halo_exchange + globals.profiler.self_halo_exchange +
                              globals.profiler.mpi_halo_exchange;
